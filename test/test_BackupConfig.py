...
def test_invalid_keep_last_n():
    with pytest.raises(ValueError, match="Keep last n Deve essere maggiore di 1 e minore di 7."):
        BackupConfig(
            backup_drive="G:",
            backup_root="BackupPC",
            source_drives=["C:", "D:"],
...
def test_invalid_keep_last_n_too_high():
    with pytest.raises(ValueError, match="Keep last n Deve essere maggiore di 1 e minore di 7."):
        BackupConfig(
            backup_drive="G:",
            backup_root="BackupPC",
...
def test_save_and_load_config():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert loaded_config.keep_last_n == config.keep_last_n
