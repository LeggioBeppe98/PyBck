...
├── Backup_C_2024-01-22_10-30-45\      # Cartelle utente
│   ├── Documents\
│   └── ...
...
class BackupBuilder:
        ...
        for drive in self.config.source_drives:
            ...
            try:
                if(drive != 'C'):
                    ...
                    self._copy_user_folders(self.config.user_folders, self.config.backup_drive, drive_folder)
    ...
    def _create_folder_drive(self, drive_letter, temp_backup_folder):
        ...
        # Codice per creare la cartella del drive
        Path(destination).mkdir(parents=True, exist_ok=True)
    ...
    def _copy_drive(self, drive_letter: str, dest_folder: Path):
        """Copia un intero drive usando py-robocopy."""
        from robocopy import Robocopy
        ...
        # Crea oggetto Robocopy
        robocopier = Robocopy(
            source=src,
            destination=dst,
            files="*",  # Tutti i file
            options=['/E', '/R:3', '/W:1', '/NP', '/NJH', '/NJS']  # TUE opzioni
        )
        
        # Esegui
        result = robocopier.run()
        
        # Gestisci risultato
        if result.returncode >= 8:  # Errori gravi
            raise Exception(f"Robocopy fallito: {result.stderr}")
        elif result.returncode > 1:  # Warning (file saltati, ecc.)
            logger.warning(f"Robocopy warning: {result.stdout}")
        
        return result

    def _copy_user_folders(self, dest_folder: Path):
        """Copia le cartelle utente specificate."""
        from robocopy import Robocopy
        ...
        for folder in self.config.user_folders:
            ...
            dst = os.path.join(dest_folder, folder)
            ...
            robocopier = Robocopy(
                ...
                destination=dst,
                files="*",
                options=['/E', '/R:3', '/W:1', '/NP', '/NJH', '/NJS']
            )
            
            result = robocopier.run()
            
            if result.returncode >= 8:
                raise Exception(f"Robocopy fallito per {folder}: {result.stderr}")